...
from DEXTR_KerasTensorflow_master.mypath import Path as Path
...
with sess.as_default():
    ...
    image = np.array(Image.open(os.path.join(os.getcwd(), 'DEXTR_KerasTensorflow_master', 'ims', 'dog-cat.jpg')))
    plt.ion()
    plt.axis('off')
    plt.imshow(image)
    ...
    while 1:
        ...
        extreme_points = extreme_points_ori - \
                         [np.min(extreme_points_ori[:, 0]), np.min(extreme_points_ori[:, 1])] + [pad, pad]
        ...
        result = helpers.crop2fullmask(pred, bbox, im_size=image.shape[:2], zero_pad=True, relax=pad) > thresh
